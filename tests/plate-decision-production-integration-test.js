...
const reloaded = JSON.parse(evaluate(`(() => {const m=__plateMatch(99702),c=__choice(m);prepareHighSchoolPlateDecision(m,c,{pitch:{pitchLocationClass:"edgeStrike",pitchType:"slider",velocity:82,movement:"breaking",location:"outer-low"},recognitionRoll:.5});player.highSchoolMatch=m;const before={pitch:m.offensivePlateAppearanceState.pendingPitch,perceived:m.activeSituation.contextSnapshot.perceivedPitch,id:m.activeSituation.situationId};player=normalizeSave(JSON.parse(JSON.stringify(player)));const after=player.highSchoolMatch;return JSON.stringify({before,after:{pitch:after.offensivePlateAppearanceState.pendingPitch,perceived:after.activeSituation.contextSnapshot.perceivedPitch,id:after.activeSituation.situationId,type:after.activeSituation.type,plate:after.plateDecisionState}});})()`));
verify("6. Save/reload before decision 保留同一 actual pitch、perception 與 situation identity", JSON.stringify(reloaded.before) === JSON.stringify({ pitch: reloaded.after.pitch, perceived: reloaded.after.perceived, id: reloaded.after.id }) && reloaded.after.type === "plateDecision" && reloaded.after.plate.situationId === reloaded.after.id);
...
verify("14. Runtime load order 在 lifecycle 後、script 前載入 Plate Decision", indexHtml.indexOf("match-situation-lifecycle.js") < indexHtml.indexOf("plate-decision-foundation.js") && indexHtml.indexOf("plate-decision-foundation.js") < indexHtml.indexOf("script.js"));
