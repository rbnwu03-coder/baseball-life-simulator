...
verify("2. 尚未播放時所有未開始半局為省略號", evaluate(`(() => {const m=__setup1222();const v=getHighSchoolMatchPresentation(m);return v.currentSituation.inning===1&&v.currentSituation.half==="上"&&v.scoreboard.away.cells[0]==="…"&&v.scoreboard.home.cells[0]==="…"&&v.scoreboard.away.cells.slice(1).every(x=>x==="…");})()`));
...
verify("4. 舊半局 bookkeeping 不改變事件游標決定的可見比分", evaluate(`(() => {const m=__setup1222();const before=JSON.stringify(getHighSchoolMatchPresentation(m).scoreboard);m.scoreboardRevealHalfIndex=2;return before===JSON.stringify(getHighSchoolMatchPresentation(m).scoreboard);})()`));
verify("5. 已播放開始事件後，進行中半局顯示即時數字", evaluate(`(() => {const m=__setup1222();advanceHighSchoolPresentationCursor(m);const v=getHighSchoolMatchPresentation(m);return v.scoreboard.away.cells[0]===0&&v.scoreboard.home.cells[0]==="…"&&v.scoreboard.away.cellStates[0]==="live";})()`));
...
verify("11. 終場已開始半局為正式數字，未打半局維持省略號，R 等於 final score", evaluate(`(() => {const m=__complete1222();const v=getHighSchoolMatchPresentation(m);return m.completed&&[v.scoreboard.away,v.scoreboard.home].every(t=>t.cells.every((cell,i)=>t.cellStates[i]==="notStarted"?cell==="…":typeof cell==="number"))&&v.scoreboard.away.visibleTotal===m.scores.away&&v.scoreboard.home.visibleTotal===m.scores.home;})()`));
...
verify("21. mixed 守備結果明確描述只完成一部分，不以抽象成敗帶過", evaluate(`(() => {const m=__reachDefense1222();m.outs=1;advanceHighSchoolYearOneAfterMomentTwo(m,"challenge","mixed");const last=m.completedMoments.at(-1);return last.resultCode==="oneOut"&&last.outcome.includes("二壘封殺成功")&&last.consequence.includes("只完成一半");})()`));
