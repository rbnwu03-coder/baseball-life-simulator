...
async function main(){
  ...
  const digest=A.signature(["5b5367c",sample,...[__filename,path.join(__dirname,"high-school-opportunity-probability-calibration-audit.cjs"),path.join(__dirname,"high-school-opportunity-probability-calibration-career.cjs")].map(p=>fs.readFileSync(p,"utf8"))]);
  ...
  })));}catch(error){for(const child of children)if(child.exitCode===null)child.kill();fs.writeFileSync(output,JSON.stringify({status:"FAIL",metadata:{baseline:"5b5367c",auditVersion:A.VERSION,sampleCount:sample},failures:[error.message],checkpointDirectory:directory},null,2)+"\n");throw error;}
