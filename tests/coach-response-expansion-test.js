...
verify("同一場交流賽包含對手、局數、出局、比分、跑者、角色、守位與任務", ["opponent", "inning", "outs", "scores", "二壘有人", "highSchoolTeamRole", "position", "assignment"].every(token => showcase.includes(token)));
verify("展示賽保留三個真正打席決策", ["attack", "zone", "advance"].every(code => showcase.includes(`matchDecision: "${code}"`)));
...
console.log("Golden High School Showcase（三種角色同場任務）：通過");
