...
const files = [
  ...
  "baseball-offense-prototype.js", "pitcher-mental-state.js", "pitcher-process-state.js", "pitch-sequencing.js", "batter-anticipation.js", "offensive-plate-approach.js", "baseball-gameplay-integration.js",
  ...
  "career-save-admission.js", "story.js", "save.js", "ai-plate-appearance-outcome.js", "script.js"
];
...
verify("57. 7局下 critical PA 同時保留 leverage 與 late-game agency", evaluate(`(() => {const m=__agencyMatch({scores:{home:1,away:2},runners:[null,"r2",null]});const c=classifyHighSchoolOffensiveOpportunity(m,__opportunityChoices());const a=evaluateHighSchoolOffensivePlayerAgency(m,c);const reached=__reachAgency(m);return c.leverageClass==="critical"&&a.lateGamePlayerAgency&&reached.result==="decision"&&reached.choices.map(x=>x.text).join()==="自己打,快速帶過這次打席";})()`));
...
verify("65. participation UI 文案只表達操作方式，沒有消極或懲罰語意", evaluate(`(() => {const m=__agencyMatch();__reachAgency(m);const labels=getHighSchoolOffensiveAgencyChoices(m).map(x=>x.text);const html=renderHighSchoolOffensiveAgencyContext(m);return labels.join()==="自己打,快速帶過這次打席"&&html.includes("想逐球判斷")&&html.includes("快速帶過")&&!/放棄|消極|扣分|能力下降|模擬/.test(html);})()`));
...
verify("124. 正式 match 初始化 match-local pitcher runtime 且不寫入 Player Truth", evaluate(`(() => {const control=player.baseballSkills.control;const m=__opaMatch({seed:94800});return m.pitcherRuntimeState?.version==="pitch-sequencing-core-sprint-a"&&m.pitcherRuntimeState.control===8&&player.baseballSkills.control===control;})()`));
verify("125. 相同 PA context 的 frozen distribution 不受 Player Approach 影響", evaluate(`(() => {const a=__opaMatch({seed:94801});const b=__opaMatch({seed:94801});const ca=getHighSchoolYearOneMatchMomentChoices(a).find(x=>x.approach==="aggressiveEarlySwing");const cb=getHighSchoolYearOneMatchMomentChoices(b).find(x=>x.approach==="patientSelection");const pa=OffensivePlateApproach.prepareNextPitch(ensureHighSchoolOffensivePlateAppearanceState(a,ca)).pendingPitch;const pb=OffensivePlateApproach.prepareNextPitch(ensureHighSchoolOffensivePlateAppearanceState(b,cb)).pendingPitch;return JSON.stringify(pa.pitcherSequencingTrace.frozenPitchDistribution)===JSON.stringify(pb.pitcherSequencingTrace.frozenPitchDistribution)&&pa.intendedPitchClass===pb.intendedPitchClass&&pa.pitchLocationClass===pb.pitchLocationClass;})()`));
verify("126. 正式無 override PA 全程使用 Pitch Sequencing canonical authority", evaluate(`(() => {const m=__opaMatch({seed:94802});const c=getHighSchoolYearOneMatchMomentChoices(m).find(x=>x.matchDecision==="zone");const s=resolveHighSchoolOffensivePlateAppearance(m,c);return s.completed&&s.pitchHistory.length>0&&s.pitchHistory.every(x=>x.pitch.generatorAuthority==="pitchSequencingCoreSprintA"&&x.pitch.pitchLocationClass===x.pitch.pitcherSequencingTrace.actualPitchClass);})()`));
verify("127. 正式 pending pitch save/reload 保留 frozen distribution、intent 與 actual", evaluate(`(() => {const m=__opaMatch({seed:94803});const c=getHighSchoolYearOneMatchMomentChoices(m).find(x=>x.matchDecision==="zone");const pending=OffensivePlateApproach.prepareNextPitch(ensureHighSchoolOffensivePlateAppearanceState(m,c));m.offensivePlateAppearanceState=JSON.parse(JSON.stringify(pending));player.highSchoolMatch=m;const loaded=normalizeSave(JSON.parse(JSON.stringify(player))).highSchoolMatch;return JSON.stringify(loaded.offensivePlateAppearanceState.pendingPitch)===JSON.stringify(pending.pendingPitch)&&JSON.stringify(loaded.pitcherRuntimeState)===JSON.stringify(m.pitcherRuntimeState);})()`));
verify("128. 正式 PA settlement 更新 match-local Mental／Process trace 但 control truth 不變", evaluate(`(() => {const m=__opaMatch({seed:94804});const control=player.baseballSkills.control;const c=getHighSchoolYearOneMatchMomentChoices(m).find(x=>x.matchDecision==="zone");const moment=resolveHighSchoolOffensiveDecision(m,c,null,{plateAppearance:{pitchSequence:[__pitch("clearBall"),__pitch("clearBall"),__pitch("clearBall"),__pitch("clearBall")],pitchOptions:Array.from({length:4},()=>({decisionRoll:.99,recognitionRoll:0}))}});return moment?.resultCode==="walk"&&m.pitcherRuntimeState.previousPAResult==="walk"&&m.pitcherRuntimeState.lastMentalTransition?.mentalStimulus==="walk"&&player.baseballSkills.control===control;})()`));
verify("129. 正式 offensive choice 顯示前已準備 Anticipation，且尚未代選 Approach", evaluate(`(() => {const m=__opaMatch({seed:94901});const before=m.batterAnticipationState;const choices=getHighSchoolYearOneMatchMomentChoices(m);return before===null&&choices.length===3&&m.batterAnticipationState?.version==="batter-anticipation-sprint-b"&&m.batterAnticipationState.chosenApproach===""&&m.prePitchPlanningState?.frozenDistribution?.finalFrozenDistribution;})()`));
verify("130. 正式 choice 只建立 readiness，不改 pre-choice frozen truth／evidence／subjective distribution", evaluate(`(() => {const m=__opaMatch({seed:94902});const choices=getHighSchoolYearOneMatchMomentChoices(m);const before=JSON.stringify({f:m.prePitchPlanningState.frozenDistribution,e:m.batterAnticipationState.observableEvidenceRaw,s:m.batterAnticipationState.subjectivePitchDistribution});ensureHighSchoolOffensivePlateAppearanceState(m,choices.find(x=>x.approach==="aggressiveEarlySwing"));return before===JSON.stringify({f:m.prePitchPlanningState.frozenDistribution,e:m.batterAnticipationState.observableEvidenceRaw,s:m.batterAnticipationState.subjectivePitchDistribution})&&m.batterAnticipationState.chosenApproach==="aggressiveEarlySwing"&&m.batterAnticipationState.prePitchReadiness?.chosenApproach==="aggressiveEarlySwing";})()`));
verify("131. 正式第一球 sampling 使用 pre-choice frozen distribution", evaluate(`(() => {const m=__opaMatch({seed:94903});const c=getHighSchoolYearOneMatchMomentChoices(m)[0];const frozen=JSON.stringify(m.prePitchPlanningState.frozenDistribution.finalFrozenDistribution);const pending=OffensivePlateApproach.prepareNextPitch(ensureHighSchoolOffensivePlateAppearanceState(m,c));return JSON.stringify(pending.pendingPitch.pitcherSequencingTrace.frozenPitchDistribution)===frozen;})()`));
verify("132. 正式 save/reload 保留 Anticipation snapshot 與 pre-pitch planning，不 reroll", evaluate(`(() => {const m=__opaMatch({seed:94904});getHighSchoolYearOneMatchMomentChoices(m);player.highSchoolMatch=m;const before=JSON.stringify({a:m.batterAnticipationState,p:m.prePitchPlanningState,e:m.pitcherObservableHistory});const loaded=normalizeSave(JSON.parse(JSON.stringify(player))).highSchoolMatch;return before===JSON.stringify({a:loaded.batterAnticipationState,p:loaded.prePitchPlanningState,e:loaded.pitcherObservableHistory});})()`));
verify("133. 正式 PA settlement 寫入 canonical observable history，下一次 evidence builder 可完整消費", evaluate(`(() => {const m=__opaMatch({seed:94905});const c=getHighSchoolYearOneMatchMomentChoices(m).find(x=>x.matchDecision==="zone");resolveHighSchoolOffensiveDecision(m,c,null,{plateAppearance:{pitchSequence:[__pitch("clearBall"),__pitch("clearBall"),__pitch("clearBall"),__pitch("clearBall")],pitchOptions:Array.from({length:4},()=>({decisionRoll:.99,recognitionRoll:0}))}});const evidence=PitchSequencing.buildObservablePitcherEvidence({observableHistory:m.pitcherObservableHistory});const anticipation=BatterAnticipation.prepareBatterAnticipation({identity:"next-production-pa",publicContext:{},observableEvidence:evidence,batterCapabilities:getHighSchoolOffensivePlateApproachAbilities(player)});return m.pitcherObservableHistory.length===4&&m.pitcherObservableHistory.every(x=>x.pitchClass==="clearBall"&&!Object.hasOwn(x,"intendedPitchClass"))&&evidence.sampleSize===4&&evidence.recentPitchClasses.every(x=>x==="clearBall")&&anticipation.observableEvidenceRaw.sampleSize===4;})()`));
verify("134. 正式 save/reload 後 canonical history 與 rebuilt evidence 完全一致", evaluate(`(() => {const m=__opaMatch({seed:94906});m.pitcherObservableHistory=[{pitchClass:"competitiveStrike",pitchResult:"calledStrike",ballsAfter:0,strikesAfter:1,commandCue:"commandAppearsStable"},{pitchClass:"edgeStrike",pitchResult:"foul",ballsAfter:0,strikesAfter:2,commandCue:"recentEdgeAttempts"},{pitchClass:"chasePitch",pitchResult:"ball",ballsAfter:1,strikesAfter:2,commandCue:"recentExpansionPattern"}];const before=PitchSequencing.buildObservablePitcherEvidence({observableHistory:m.pitcherObservableHistory});player.highSchoolMatch=m;const loaded=normalizeSave(JSON.parse(JSON.stringify(player))).highSchoolMatch;const after=PitchSequencing.buildObservablePitcherEvidence({observableHistory:loaded.pitcherObservableHistory});return JSON.stringify(loaded.pitcherObservableHistory)===JSON.stringify(m.pitcherObservableHistory)&&JSON.stringify(after)===JSON.stringify(before)&&after.sampleSize===3&&after.previousPitchResult==="ball";})()`));
verify("135. aggressive 追打文案歸因 Recognition error 而非 approach 本身", evaluate(`(() => {const h=[{pitchNumber:1,pitch:{pitchLocationClass:"chasePitch",strike:false},recognition:{correct:false,perceivedPitchClass:"competitiveStrike"},selectionProfile:"aggressive",countBefore:{balls:0,strikes:0},countAfter:{balls:0,strikes:0},action:"swing",contact:true,pitchResult:"ballInPlay"}];const s=OffensivePlateApproach.createPlateAppearanceState({paIdentity:"semantic-presentation-chase",approach:"aggressiveEarlySwing",pitchHistory:h,pitchNumber:1,completed:true,result:"out",swingExecutionSummary:{swings:1,takes:0,whiffs:0,fouls:0,ballsInPlay:1,hardContacts:0}});const text=formatHighSchoolOffensiveExecutionText(__presentationChoice("aggressive"),s);return text.includes("判成了可攻擊球")&&!text.includes("因為你選擇積極");})()`));
verify("136. narrow-zone 正確放掉 edgeStrike 不被呈現為決策失誤", evaluate(`(() => {const h=[{pitchNumber:1,pitch:{pitchLocationClass:"edgeStrike",strike:true},recognition:{correct:true,perceivedPitchClass:"edgeStrike"},selectionProfile:"selective",countBefore:{balls:0,strikes:0},countAfter:{balls:0,strikes:1},action:"take",contact:null,pitchResult:"calledStrike"}];const s=OffensivePlateApproach.createPlateAppearanceState({paIdentity:"semantic-presentation-edge",approach:"patientSelection",pitchHistory:h,pitchNumber:1,strikes:1,swingExecutionSummary:{swings:0,takes:1,whiffs:0,fouls:0,ballsInPlay:0,hardContacts:0}});const text=formatHighSchoolOffensiveExecutionText(__presentationChoice("selective"),s);return text.includes("不是你設定要積極攻擊的位置")&&!text.includes("失誤");})()`));
