...
test("legal optional types share pool before draw",()=>{const r=select(plan(fixture()));assert(r.probabilityResult.weights.some(w=>w.provenance.campSourceId));assert(r.probabilityResult.weights.some(w=>!w.provenance.campSourceId));assert(!r.rejectedCandidates.some(c=>c.reasons.includes("typeConflict")));});
