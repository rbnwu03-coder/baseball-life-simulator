...
verify("13. 二壘手 4-6-3 由玩家第一傳、游擊手補位轉傳", evaluate(`(() => {const c=getHighSchoolDefensiveMomentChoices(__infield21("二壘手", "normalGrounder", {direction:"upTheMiddle"}));const dp=c.find(x=>x.routeId==="initiate463");return dp&&dp.action==="throwSecond"&&dp.text.includes("補位游擊手")&&!c.some(x=>x.infieldRoute==="forceSecond");})()`));
...
verify("42. featured match 使用 live-state meaningful-moment extension point", evaluate(`(() => {const m=__setup21();let safety=0;while(!isHighSchoolMatchDecisionVisible(m)&&safety++<160)advanceHighSchoolMatchPlaybackStep(m);resolveHighSchoolYearOneMatch("zone",m.currentMomentId,()=>.8);while(isHighSchoolMatchPlaybackPhase(m)&&safety++<320){const result=advanceHighSchoolMatchPlaybackStep(m);if(result==="decision")break;}return m.currentDomain==="defense"&&m.positionDecisionFamily==="infield"&&m.defensiveSituation.id.includes(m.id)&&m.currentAssignment.includes("滾地球");})()`));
