...
verify("23. 三個 proof moments 都從同一場 live match 進入並完成", evaluate(`(() => {const m=__run22().match;return m.completedMoments.length===3&&new Set(m.completedMoments.map(x=>x.id)).size===3&&m.completedMoments.every(x=>x.inning>=1);})()`));
