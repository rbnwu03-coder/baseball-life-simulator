...
function makeContext() {
  ...
  vm.runInContext(`
    ...
    function __alignPlayerEntrySlot22(match) {
      const assignedPosition=match.gameExposureState?.opportunitySnapshot?.assignedPosition||match.developmentPositionOverride||match.position;
      const incumbent=getCurrentHighSchoolMatchDefender(match,"home",assignedPosition);
      const slot=match.rosters.home.lineup.indexOf(incumbent);
      if(slot<0)throw new Error("Fixture requires canonical substitution incumbent");
...
verify("18. 板凳登場沿用當下比分、出局與跑者", evaluate(`(() => {const m=__setup22("bench");Object.assign(m,{inning:5,half:"下",offenseTeam:"home",defenseTeam:"away",outs:1,scores:{home:2,away:3},runners:["home-sim-2",null,"home-sim-4"]});__alignPlayerEntrySlot22(m);const truth=JSON.stringify({i:m.inning,h:m.half,o:m.outs,s:m.scores,r:m.runners});enterHighSchoolMatchPlayer(m);return truth===JSON.stringify({i:m.inning,h:m.half,o:m.outs,s:m.scores,r:m.runners})&&m.playerLineupStatus==="substitute"&&m.rosters.home.lineup[m.playerLineupSlot].id==="player"&&m.playerFieldingAssignment;})()`));
verify("19. 登場事件保存 live-state snapshot", evaluate(`(() => {const m=__setup22("rotation");Object.assign(m,{inning:4,half:"下",offenseTeam:"home",defenseTeam:"away",outs:2,scores:{home:1,away:2},runners:[null,"home-sim-3",null]});__alignPlayerEntrySlot22(m);const e=enterHighSchoolMatchPlayer(m);return e.type==="playerEntry"&&e.presentationSnapshot.inning===4&&e.presentationSnapshot.outs===2&&e.presentationSnapshot.runners[1]==="home-sim-3"&&e.presentationSnapshot.scores.away===2;})()`));
