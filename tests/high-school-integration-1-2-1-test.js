...
function makeContext() {
  ...
  vm.runInContext(`
    ...
    function __complete121(role="starter", sample=0.99) {
      ...
      advanceHighSchoolMatchSimulation(player.highSchoolMatch);
...
verify("25. 守備後若 Regulation 尚未終場，推進到終盤且打序回到玩家的 Moment 3", evaluate(`(() => {const match=__setup121();__resolveAdvance121("zone",0.99);__resolve121("secure",0.99);__forceThirdMoment121();advanceHighSchoolMatchSimulation(match);return match.simulationPhase==="moment_3_ready"&&match.momentIndex===2&&match.inning>=5&&match.half==="下"&&match.offenseTeam==="home"&&match.currentBatter==="player";})()`));

verify("26. Moment 3 任務會依 simulator 的落後、平手、領先比分切換", evaluate(`(() => {const match=__setup121();match.momentIndex=2;match.runners=[null,null,null];match.outs=1;const texts=[];for(const scores of [{home:1,away:3},{home:2,away:2},{home:4,away:2}]){match.scores=scores;texts.push(getHighSchoolYearOneMatchMomentChoices(match).map(item=>item.text).join("｜"));}return texts[0].includes("縮小差距")&&texts[1].includes("超前")&&texts[2].includes("保險分");})()`));

verify("27. Moment 3 寫回後由 simulator fast-forward 到終場並結算一次", evaluate(`(() => {const match=__complete121("starter");return match.completed&&match.settled&&match.simulationPhase==="complete"&&match.half==="終"&&match.outs>=0&&match.outs<=3&&match.currentMomentId===""&&match.completedMoments.length===3&&match.teamResult.includes("終場");})()`));
...
verify("34. Defense outcome reload 後只 resume 一次，不重複該次守備", evaluate(`(() => {const match=__setup121();__resolveAdvance121("zone",0.99);__resolve121("secure",0.99);__forceThirdMoment121();const completed=match.completedMoments.length;const contribution=match.playerContribution.outsCreated;saveGame();player=createInitialPlayer();loadGame();return player.highSchoolMatch.simulationPhase==="moment_3_ready"&&player.highSchoolMatch.completedMoments.length===completed&&player.highSchoolMatch.playerContribution.outsCreated===contribution;})()`));
...
const roleProof = parse(`(() => ["starter","rotation","bench"].map(role=>{const match=__complete121(role);return {role,completed:match.completed,moments:match.completedMoments.length,domains:match.completedMoments.map(item=>item.domain),result:match.teamResult};}))()`);
verify("37. Starter／Rotation／Bench 都能依 Regulation 走完同一套 simulation gameplay", roleProof.every(item => item.completed && item.moments >= 1 && item.moments <= 3 && item.domains[0] === "offense" && item.domains.includes("defense") && item.result.includes("終場")));
